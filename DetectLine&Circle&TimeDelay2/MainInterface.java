...
public class MainInterface extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource()==opencam){
			...
			/*try{
				ImageIO.write(processingPic1(webcam.getImage()),"png",new File("test.png"));
	...
	public boolean detectLine(BufferedImage bi){		//第一阶段测试，判断是否为直线，返回检测结果，需要对拍照所得图片处理，参数传入的应为拍摄图片
		...
		BufferedImage processed = processingPic1(bi);	//对拍摄图片进行处理
	...
	public boolean detectCircle(BufferedImage bi){		//判断是否为圆，并返回检测结果,当前为测试的第二阶段
		...
		BufferedImage processed = processingPic1(bi);	//处理图片
	...
	*/
	...
	public BufferedImage processingPic1(BufferedImage before){	//首先将黑色变白，然后将红色变黑,非红色变白
		int width = before.getWidth();
		int height = before.getHeight();							
		int minx = before.getMinX();
		int miny = before.getMinY();
		...
		for(int i = minx;i<width;i++){
			for(int j = miny;j<height;j++){
				...
				rgb[2] = (pixel & 0xff);
				int gray = (rgb[0]*30+rgb[1]*59+rgb[2]*11+50)/100;		//灰度计算公式
				if(gray<=45&&(rgb[0]-rgb[1])<=25&&(rgb[0]-rgb[2])<=25){					//检测黑块，黑色像素的灰度相较于白色，容易发现
					after.setRGB(i,j,new Color(255,255,255).getRGB());
				}else{
					after.setRGB(i,j,before.getRGB(i,j));	//其他部分保留原色
				}
			}
		}
		for(int i = minx;i<width;i++){			//将白色部分变白，其余部分变黑
			for(int j = miny;j<height;j++){
				int pixel = after.getRGB(i,j);
				rgb[0] = (pixel & 0xff0000)>>16;                                         
				rgb[1] = (pixel & 0xff00)>>8;
				rgb[2] = (pixel & 0xff);
				int gray = (rgb[0]*30+rgb[1]*59+rgb[2]*11+50)/100;
				if(gray-rgb[0]<=30&&gray-rgb[1]<=30&&gray-rgb[2]<=30&&(rgb[0]-rgb[1])<=30||(rgb[0]-rgb[2])<=30)	//白色的RGB分量与灰度很相似，且R值与G值和B值相近
					after.setRGB(i,j,new Color(255,255,255).getRGB());
				else
					after.setRGB(i,j,new Color(0,0,0).getRGB());
			}
		}
		for(int i = minx;i<width;i+=20){	//对图像进行降噪处理
			for(int j = miny;j<height;j+=20){
				noiseReduction(after,i,j,20,20,15);
			}
		}
		return after;
	}

	public void noiseReduction(BufferedImage bi,int x,int y,int width,int height,int numbers){         //矩形范围降噪
		...
		if(count<numbers){
			setWhite(bi,x,y,width,height);
		}
	}

	public void setWhite(BufferedImage bi,int x,int y,int width,int height){         ///将这块区域变白
			...
			for(;;){
				if(samFinished){
					...
					for(int i = 0;i<animashot.size();i++){
						animashot.set(i,processingPic1(animashot.get(i)));		//对采样图片进行二值化处理
						/*try{
		...
		public void run(){
			for(;;){
				if(level3shot!=null){		//等待测试三拍摄图片
					...
					shotpanel.add(shotimg);
					shotpanel.repaint();
					level3shot = processingPic1(level3shot);		//对拍摄的图片进行处理(二值化)
